...
export default function Carrito() {
  ...
  const aplicarCupon = async () => {
    ...
    if (inputCupon.trim() === "") {
      setCuponError("Ingresa un cupón válido.");
      return;
    }

    try {
      const cuponBuscado = inputCupon.trim().toUpperCase();

      // Buscar el cupón en Firestore
      const q = query(
        collection(db, "cupones"),
        where("codigo", "==", cuponBuscado)
      );

      const snap = await getDocs(q);

      if (snap.empty) {
        setCuponError("El cupón ingresado no existe.");
        return;
      }
      ...
      if (!cupon.activo) {
        setCuponError("Este cupón ya no está activo.");
        return;
      }
      ...
      const fechaExp = cupon.expiracion.toDate
        ? cupon.expiracion.toDate()
        : new Date(cupon.expiracion);

      if (fechaExp < hoy) {
        setCuponError("Este cupón ya expiró.");
        return;
      }

      // Validar límite de usos
      if (cupon.limiteUso > 0 && cupon.usosActuales >= cupon.limiteUso) {
        setCuponError("Este cupón alcanzó su límite de usos.");
        return;
      }

      // Aplicar descuento
      const valorDescuento = subtotal * (cupon.descuento / 100);
        ...
        (t) => (
          <div
            className={`${t.visible ? "animate-enter" : "animate-leave"
              } bg-[#1A1F25] border border-[#00C8D7] text-white p-4 rounded-lg shadow-lg flex items-start max-w-xs`}
          >
            <CheckCircle className="text-green-400 w-6 h-6 mr-3 mt-1" />

            <div className="flex-1">
              <p className="text-base font-bold">Cupón aplicado</p>
              <p className="text-sm text-gray-300">{cupon.codigo} — {cupon.descuento}% de descuento</p>
            </div>
      ...
      setCuponError("Ocurrió un error al validar el cupón.");
    }
  };
              ...
              <div className="pt-2 border-t border-gray-700">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className={`w-full p-2 rounded-lg text-sm bg-gray-700 border
    ${cuponAplicado ? 'opacity-60 cursor-not-allowed' : cuponError ? 'border-red-500' : 'border-gray-700 focus:border-[#00C8D7]'}
    text-white placeholder-gray-400 transition`}
                  ...
                  {cuponAplicado ? (
                    <button
                      ...
                      className="py-2 px-3 rounded-lg text-sm font-bold bg-red-600 text-white hover:bg-red-700 transition flex items-center"
                    >
                    ...
                    <button
                      ...
                      disabled={inputCupon.trim() === ""}
                      className="py-2 px-3 rounded-lg text-sm font-bold bg-[#00C8D7] text-gray-900 hover:bg-[#00E0FF] transition flex items-center"
                    >
