...
export default function PeliDocumentales() {
  ...
  // 🔥 FIRESTORE: Documentales
  useEffect(() => {
    const q = query(
      collection(db, "peliculas"),
      where("categoria", "==", "Documentales")
    );
  ...
  // 🎨 ESTILOS (idénticos a los otros módulos)
  const btnBase =
    ...
    >
      ...
      <div className="md:grid md:grid-cols-4 md:gap-8">
        ...
        <main className="md:col-span-3">
          <h2 className="text-3xl font-bold mb-4">Contenido</h2>

          {loading ? (
            <p className="text-gray-400">Cargando documentales...</p>
            ...
            <div className="flex flex-col items-center justify-center h-48 border-2 border-dashed border-gray-500/50 rounded-xl">
              <p className="text-xl font-semibold text-gray-400">
                No hay documentales de "{generoSeleccionado}" disponibles.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
              {peliculasFiltradas.map((docu) => (
                <div
                  key={docu.id}
                  className={`rounded-xl overflow-hidden shadow-2xl hover:scale-[1.03] transition-transform duration-300 ${
                    theme === "dark" ? "bg-[#1A1F25]" : "bg-white"
                  }`}
                >
                  <Link to={`/detalle/${docu.id}`}>
                    <img
                      src={docu.imagen}
                      alt={docu.titulo}
