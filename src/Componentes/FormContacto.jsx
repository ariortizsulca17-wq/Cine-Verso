...
const SuccessModal = ({ isOpen, onClose, theme }) => {
  ...
  const modalBg = theme === 'dark' ? 'bg-gray-800' : 'bg-gray-200';
  const textColor = theme === 'dark' ? 'text-white' : 'text-gray-900';
  const subTextColor = theme === 'dark' ? 'text-gray-400' : 'text-gray-700';

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className={`${modalBg} p-8 rounded-xl shadow-2xl max-w-sm w-full border border-green-500/50 relative text-center`}>
        <button 
          onClick={onClose} 
...
export function FormContacto() {
  ...
  const labelText = theme === 'dark' ? 'text-gray-300' : 'text-gray-800';
  const iconColor = theme === 'dark' ? 'text-cyan-400' : 'text-cyan-600';
  const btnTextDark = theme === 'dark' ? 'text-gray-900' : 'text-white';
  ...
  return (
    <>
      <form 
        ...
        className={`space-y-6 p-8 rounded-xl shadow-2xl border border-cyan-600/30 ${formBg}`}
      >
        {/* Nombre */}
        <div>
          <label htmlFor="name" className={`block text-sm font-medium mb-1 ${labelText}`}>
            <User className={`w-4 h-4 inline mr-1 ${iconColor}`} /> Nombre Completo
          </label>
          <input
            type="text"
            id="name"
            ...
            required
            className={`w-full px-4 py-2 ${inputBg} border ${inputBorder} rounded-lg ${inputText} focus:ring-cyan-500 focus:border-cyan-500 transition duration-150 text-sm`}
        ...
        <div>
          <label htmlFor="email" className={`block text-sm font-medium mb-1 ${labelText}`}>
            <Mail className={`w-4 h-4 inline mr-1 ${iconColor}`} /> Correo Electrónico
          </label>
          <input
            type="email"
            id="email"
        ...
        {/* Mensaje */}
        <div>
          <label htmlFor="message" className={`block text-sm font-medium mb-1 ${labelText}`}>
            <MessageSquare className={`w-4 h-4 inline mr-1 ${iconColor}`} /> Mensaje
          </label>
          <textarea
            id="message"
            name="message"
            ...
            onChange={handleChange}
            required
            className={`w-full px-4 py-2 ${inputBg} border ${inputBorder} rounded-lg ${inputText} focus:ring-cyan-500 focus:border-cyan-500 transition duration-150 text-sm resize-none`}
        ...
        <button
          ...
          className={`w-full flex items-center justify-center font-bold py-3 px-4 rounded-lg transition duration-300 uppercase tracking-wider text-sm ${
            isSending 
              ...
              : `bg-cyan-600 ${btnTextDark} hover:bg-cyan-500 shadow-lg shadow-cyan-500/50`
          }`}
        >
          {isSending ? (
            <>
