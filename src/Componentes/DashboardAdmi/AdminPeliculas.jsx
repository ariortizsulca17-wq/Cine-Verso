...
export default function AdminPeliculas() {
  ...
  const [mensajeExito, setMensajeExito] = useState(""); // Para notificaciones de éxito
  ...
  const eliminarPelicula = async () => {
    if (!modal.pelicula) return;

    setProcesando(true);
    setError(null);

    try {
      const peliculaId = modal.pelicula.id.toString();
      await deleteDoc(doc(db, "peliculas", peliculaId));

      // Actualizar el estado para reflejar la eliminación
      setPeliculas((prev) => prev.filter((p) => p.id !== modal.pelicula.id));
      setFiltradas((prev) => prev.filter((p) => p.id !== modal.pelicula.id));

      // 🚀 Mensaje de éxito
      setMensajeExito(`La película "${modal.pelicula.titulo}" fue eliminada correctamente.`);
  ...
  return (
    <div className="p-6 text-white max-w-7xl mx-auto font-sans">
      ...
      {/* MENSAJE DE ÉXITO */}
      {mensajeExito && (
        <div className="bg-green-900/40 border border-green-500 text-green-300 p-4 rounded-lg mb-6 flex items-center shadow-lg transition-all">
